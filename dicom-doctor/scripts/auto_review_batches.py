...
v2.11.0 Harness 增强：
- 单条级别自动重试（最多 N 次，指数退避）
- 输出截断检测（JSON 括号不匹配、输出过短）
- 假完成检测（模型提前输出总结性语句）
- max_tokens 自适应（防止模型输出被截断）
...
class OpenAICompatibleVisionClient:
    ...
    def _single_call(self, item: Dict) -> Tuple[str, str]:
        """单次 API 调用，返回 (text, finish_reason)。"""
        payload = {
            "model": self.model,
        ...
        try:
            ...
            raise RuntimeError(f"HTTP {exc.code}: {detail_text}") from exc
    ...
    def review_item(self, item: Dict) -> str:
        """带重试、截断检测、假完成检测的阅片调用。"""
        from review_harness import detect_truncation, detect_fake_completion
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                text, finish_reason = self._single_call(item)

                # 截断检测
                is_truncated, reason = detect_truncation(text)
                if is_truncated:
                    logger.warning(
                        "[Harness增强] %s 第 %s/%s 次：输出截断 — %s (finish_reason=%s)",
                        png_name, attempt, self.max_retries, reason, finish_reason,
                    )
                    if attempt < self.max_retries:
                        time.sleep(2.0 * (2 ** (attempt - 1)))
                        continue
                    # 最后一次仍截断，返回截断的文本让上层尝试解析
                    logger.warning("[Harness增强] %s 重试 %s 次后仍截断，返回截断文本", png_name, self.max_retries)
                    return text

                # 假完成检测
                is_fake, pattern = detect_fake_completion(text)
                if is_fake:
                    logger.warning(
                        "[Harness增强] %s 第 %s/%s 次：检测到假完成信号 — '%s'",
                        png_name, attempt, self.max_retries, pattern,
                    )
                    if attempt < self.max_retries:
                        time.sleep(2.0 * (2 ** (attempt - 1)))
                        continue
                    # 最后一次仍有假完成信号，返回文本让上层尝试解析
                    logger.warning("[Harness增强] %s 重试 %s 次后仍有假完成信号，返回文本", png_name, self.max_retries)
                ...
                last_error = exc
                logger.warning(
                    "[Harness增强] %s 第 %s/%s 次调用失败: %s",
                    png_name, attempt, self.max_retries, exc,
                )
                if attempt < self.max_retries:
                    time.sleep(2.0 * (2 ** (attempt - 1)))

        raise last_error or RuntimeError(f"{png_name} 重试 {self.max_retries} 次后仍失败")
...
def parse_args():
    ...
    parser.add_argument("--max-retries", type=int, default=3, help="单条最大重试次数（v2.11.0 Harness 增强），默认 3")
    parser.add_argument("--max-tokens", type=int, default=None, help="模型最大输出 token 数（v2.11.0 Harness 增强），不设则由模型默认")
