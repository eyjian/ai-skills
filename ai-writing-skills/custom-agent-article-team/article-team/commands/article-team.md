---
description: 启动自定义 Agent 文章编写团队。通过 CodeBuddy 的 team_create / task（异步团队模式，使用自定义注册 Subagent）/ send_message / team_delete 工具链，创建 5 个独立自定义 Agent 实例以网状拓扑协作。支持 AI、健康、跑步及其他可配置领域的新稿创作、旧稿重审、旧稿优化、旧稿修改 / 回炉、旧稿直接润色。
...
- reviewer 报告有 🔴 问题 → 立即派发 **writer**（`subagent_name: "article-writer"`），在 prompt 中注入审稿报告和修改要求
- reviewer 报告无 🔴 问题 → 直接派发 **polisher**（`subagent_name: "article-polisher"`），做终稿润色
...
即使用户只要求润色，也先派发 **reviewer 做快速技术检查**——确保没有技术错误后再交给 polisher。
...
- 无严重问题 → 派发 **polisher**（`subagent_name: "article-polisher"`），在 prompt 中注入 reviewer 的快审结论
- 有严重问题 → 先派发 **writer**（`subagent_name: "article-writer"`）修改，修改完再派发 polisher
...
## 第 5 步：等待 Agent 反馈并按需派发后续 Agent
...
2. 用户确认后，派发 architect：
   - 调用 `task(subagent_name: "article-architect", name: "architect", team_name: "article-team", ...)` 派发
...
### 收到 polisher 的终稿完成通知
1. **🚨 协调者必须亲自验证"你"字清零**：用 `search_content` 对目标文件搜索 `你`，逐个检查搜索结果——对读者说的"你"必须为 0。
   ...
   - 判定方法：对每个"你"问一个问题——**"这个'你'指的是谁？"**。如果答案是"正在读文章的人"，那就是有害的
2. 如果对读者说的"你" > 0，通过 `send_message` 打回 polisher 要求清零，附带具体行号和原文
3. **循环验证**：polisher 修改后再次搜索验证，直到对读者说的"你"为 0 才放行
4. **⛔ 此步骤不可跳过**：即使 reviewer 和 polisher 都报告"你"字已清零，协调者仍必须亲自执行 search_content 验证。这是最终关卡
5. 通过后展示终稿给用户确认
...
- **风格基线参照**：如果目标文章属于某个系列（同目录下有其他 `.md` 文章），协调者必须扫描同目录找到 1-2 篇同系列文章的路径，作为"风格基线参照文件"注入每个 Agent 的 prompt。格式：`风格基线参照文件：{文件路径1}、{文件路径2}——Agent 须先读取参照文件前 50-100 行建立风格基线，确保产出与系列风格一致。`
...
9. **`subagent_name` 使用各角色的自定义注册名**：`"article-scout"` / `"article-architect"` / `"article-writer"` / `"article-reviewer"` / `"article-polisher"`。这些是在 `.codebuddy/agents/` 中注册的自定义 Subagent，角色的 System Prompt 由平台自动加载，协调者只需在 prompt 中注入运行时上下文（不再需要 read_file 读取 agents/*.md）
10. **prompt 只注入运行时上下文**：方式 B 中，角色定义已由平台通过 Subagent 注册自动加载。协调者 prompt 中只需注入：领域画像解析结果、任务模式、前序产出、目标文件路径、风格基线参照等运行时信息
...
14. **🚨"你"味清零是硬卡控，不是软建议**：协调者是"你"字清零的最终关卡。每个 Agent 完成工作后、协调者在派发下一个 Agent 之前，都应该用 `search_content` 验证目标文件中对读者说的"你"是否已清零。尤其是 writer 改稿完成后、polisher 润色完成后这两个关键节点。不清零不放行
