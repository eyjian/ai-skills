...
- 💡 **激活**：任务卡从协调者飞到工位，桌灯亮起
- ⌨️ **工作中**：打字动画，桌面纸张翻动
- ✉️ **正式交付**：纸飞机沿实线弧线飞向接收者
- 💬 **即时讨论**：对话气泡沿虚线弹跳传递，发送者和接收者同时进入 talking 状态
- 🔔 **知会/FYI**：通知图标沿点线飘向接收者，不触发状态切换
- ✅ **完成**：绿色对勾弹出，角色伸懒腰
...
- 三种消息类型用不同视觉区分：
  - **✉️ 正式交付**（`message`）：标准气泡样式
  - **💬 即时讨论**（`discussion`）：虚线左边框 + 淡紫背景 + 💬 标签
  - **🔔 知会/FYI**（`notification`）：点线左边框 + 淡灰背景 + 🔔 标签
- 系统事件（团队创建、成员加入、任务完成）以居中灰色文本显示
- 广播消息以蓝色边框突出显示
- 长消息自动折叠，点击"展开"查看全文
- 支持**按消息类型过滤**：✉️ 正式 / 💬 讨论 / 🔔 知会
