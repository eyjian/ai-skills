...
def _pip_install(package_spec: str) -> bool:
    ...
    Returns:
        是否安装成功
    """
    logger.info(f"[自修复] 正在安装缺失依赖: {package_spec} ...")
    try:
        ...
        if result.returncode == 0:
            logger.info(f"[自修复] 安装成功: {package_spec}")
            return True
        else:
            logger.warning(
                f"[自修复] 安装失败: {package_spec}\n"
                f"  stdout: {result.stdout.strip()}\n"
                f"  stderr: {result.stderr.strip()}"
            )
            return False
    except subprocess.TimeoutExpired:
        logger.warning(f"[自修复] 安装超时: {package_spec}")
        return False
    except Exception as e:
        logger.warning(f"[自修复] 安装异常: {package_spec}: {e}")
        return False
...
def run_pipeline(input_path: str, output_dir: str = None, enhance: bool = False,
    ...
    if review_results_json:
        ...
        if not model_capable:
            ...
            review_results = reviewer.review(png_files, export_dir=str(output_dir), cad_hint=cad_prompt_hint)
